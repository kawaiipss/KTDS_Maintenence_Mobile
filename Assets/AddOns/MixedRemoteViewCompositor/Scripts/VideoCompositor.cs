...
namespace MixedRemoteViewCompositor
{
    ...
    public class VideoCompositor : MonoBehaviour
    {
        ...
        private Material compositorMaterial2 = null;    //*LKH* - 추가
        private Texture2D mTexture2D;   //*LKH* - 추가
        public RenderTexture mAddRenderTexture; //*LKH* - 추가
        public Action<Texture2D, UnityEngine.Matrix4x4> mCaptureUpdateCallback; //*LKH* - 추가
        public string mRunningTextureName;  //*LKH* - 추가
        ...
        {
            //if (this.mAddRenderTexture == null || CRemoteProcess.Instance.mVideoCaptureState == false || mRunningTextureName == "" || this.compositorMaterial2 == null)  //*LKH* - 추가
            //    return;
            ...
            */
            ...
            //*LKH* - 추가--------------------------------------
            var upSideDown = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
            Graphics.Blit(source, upSideDown, TextureScale, TextureOffset);

            var temp = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);

            this.compositorMaterial2.SetFloat("_Brightness", 1);
            this.compositorMaterial2.SetFloat("_Transparency", 1);
            this.compositorMaterial2.SetTexture("_VideoTex", upSideDown);
            this.compositorMaterial2.SetTextureOffset("_VideoTex", this.TextureOffset);
            this.compositorMaterial2.SetTextureScale("_VideoTex", this.TextureScale);
            Graphics.Blit(this.mAddRenderTexture, temp, this.compositorMaterial2, 0);
            ...
            this.compositorMaterial.SetTextureOffset("_VideoTex", this.TextureOffset);
            this.compositorMaterial.SetTextureScale("_VideoTex", this.TextureScale);
            Graphics.Blit(temp, destination, this.compositorMaterial, 0);

            //mTexture2D = destination.toTexture2D();
            if (mCaptureUpdateCallback != null)
                mCaptureUpdateCallback(mTexture2D, this.defaultCameraProjection);

            RenderTexture.ReleaseTemporary(upSideDown);
        ...
        private void CreateResources()
        {
            ...
            if (this.compositorMaterial == null)
            {
                ...
                this.compositorMaterial2 = new Material(this.CompositorShader); //*LKH* - 추가
            }
        ...
        private void DestroyResources()
        {
            if (this.compositorMaterial != null)
            {
                ...
                Destroy(this.compositorMaterial2);  //*LKH* - 추가
                this.compositorMaterial2 = null;    //*LKH* - 추가
            }
        }
